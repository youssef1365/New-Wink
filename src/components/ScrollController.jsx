    ...
    const handleScroll = () => {
      if (!ticking) {
        requestAnimationFrame(() => {
          ...
          const smoothed = velocity.get() * smooth + Math.min(v, cap) * react;
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (visible) setActiveSection(visible.target.dataset.sectionId);
      },
      { rootMargin: "-25% 0% -25% 0%", threshold: [0.1, 0.25, 0.5, 0.75] }
    );
  ...
  return (
    <>
      ...
      <div id="about" data-section-id="about" ref={reg("about")}>
        <AboutUs isActive={activeSection === "about"} scrollVelocity={velocity} />
      </div>

      <div data-section-id="vision1" ref={reg("vision1")}>
        <Vision
          isActive={activeSection === "vision1"}
          image="/transition1.jpeg"
          lines={[
            { words: 'POWERED BY' },
            { words: 'SMART TECHNOLOGIES', accent: true },
          ]}
          subtext="Wink integrates advanced matchmaking systems and structured meeting management tools to ensure precision, scalability, and efficiency — enhancing human expertise with intelligent infrastructure."
        />
      </div>

      <div id="services" data-section-id="services" ref={reg("services")}>
        <Services isActive={activeSection === "services"} scrollVelocity={velocity} />
      </div>

      <div data-section-id="vision2" ref={reg("vision2")}>
        <Vision
          image="/transition2.jpeg"
          lines={[
            { words: 'EVERY MEETING IS' },
            { words: 'DESIGNED TO CREATE' },
            { words: 'GROWTH PARTNERSHIPS', accent: true },
            { words: 'AND LONG-TERM' },
            { words: 'IMPACT.', accent: true },
          ]}
        />
      </div>
